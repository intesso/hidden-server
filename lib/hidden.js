...
HiddenServer.prototype.check = function() {
  var self = this;
  var interval = self.get('pingInterval') * 1000;
  var max = interval * self.get('checkInterval');
  this.checkHandler = setInterval(function() {
    var now = Date.now();
    if (!self.lastPing || (now - max) > self.lastPing) {
      self.init();
    }
  }, interval);
};
