...
HiddenServer.prototype.sendPingRequest = function(obj) {
  var self = this;
  var simPings = this.get('simultaneousPings');
  if (obj) this.callbackCount--;
  obj = obj || {};
  if (this.pingCount >= (simPings - this.callbackCount)) return false;
...
HiddenServer.prototype.receivePingResponse = function(err, res) {
  ...
  if (err) return self.emit('httpRequestError', err);
  ...
  if (res.body && body.command) {
    callbackCount++;
    self.emit('command', body, self.sendPingRequest);
  }
