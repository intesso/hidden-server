...
PublicServer.prototype.handleRequest = function(requests, name, res, message) {
  ...
  if (!this.get('roundTripResponse')) {
    ...
    this.saveRequest(this.roundTripRequests, name, res.req, message, this.get('pingInterval'));
  }
...
PublicServer.prototype.saveRequest = function(requests, name, req, message, timeout) {
  ...
  if (this.get('roundTripResponse')) {
    ...
    // TODO check first before popping from array
    var i = this.findRequest(rtrs, obj.id);
    debug('idTest', obj.id, i, rtrs);
    if (~i) {
      debug('idTestTrue', rtrs[i], i, obj.id);
      // respond to the saved request with the same id
