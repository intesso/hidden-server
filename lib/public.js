...
PublicServer.prototype.handleRequest = function(requests, name, res, message) {
  ...
  if (!this.get('roundTripResponse')) {
    ...
    this.saveRequest(this.roudTripRequests, name, res.req, message, self.get('pingInterval'));
  }
...
PublicServer.prototype.saveRequest = function(requests, name, req, message, timeout) {
  ...
  // TODO respond to the original request | only respond the actual request once the response of the original request has been received
